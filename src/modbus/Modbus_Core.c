...
void MBS_CorePoll()
{
    ...
    {
        if (MBS_Buf._rxBuff[MBS_FRAME_SLAVER_ADDR] != MBS_SelfAddr && Lora_Para_AT.Enable == 1)
        { // 非本机地址，数据透传
            if (mb_host.state == MBS_STATE_IDLE)
            {
                mb_port_send_data(MBS_Buf._rxBuff, MBS_Buf._rxLen);
                // MBS_Physical2SendBuff(MBS_Buf._rxBuff, MBS_Buf._rxLen);
                mb_host.isSelfcmd = false;
            }
            else
                ringbuff_write_Hex(&Z9_ringbuf, MBS_Buf._rxBuff, MBS_Buf._rxLen);
        }
        else if (MBS_Buf._rxBuff[MBS_FRAME_SLAVER_ADDR] == MBS_RoomCtrlAddr || MBS_Buf._rxBuff[MBS_FRAME_SLAVER_ADDR] == MBS_SelfAddr)
        { // 本机地址，数据处理
            MBS_CoreAnalyze();
        }
    }
